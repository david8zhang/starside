...
public class AimerHorizontal : MonoBehaviour {
    ...
    /** Initializes Board size and creates the aimer pieces that make up the horizontal crosshair. */ 
	void Awake() {
    ...
    /** Create the crosshair piece by piece. */
    ...
    /** Update the crosshair position along the board */ 
	void Update() {
		// use Mathf.PingPong() to make the aimer move back and forth along the board
		if (aiming && !paused) {
